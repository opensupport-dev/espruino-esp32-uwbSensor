...
Serial2.on('data', function(data) {
  ...
  else
  {
    ...
    else
    {
      ...
      var s=data.split(',');
      if (isNaN(parseInt(s[0],10))){T.value1=undefined;} else {T.value1=parseInt(s[0],10);TT.m.push(T.value1);}
      if (isNaN(parseInt(s[1],10))) {T.value2=undefined;} else {T.value2=parseInt(s[1],10);TT.b.push(T.value2);}
      if (f.protocol=='mqtt'  && ( block_communications === 0) && (busy === 0)){ ToMQTT(T);} 
...
function WebSetup(res) {
  ...
  else if (f.mqtt_timeout == 60){ S3 = 'selected'; }
  else { S2 = 'selected'; }
  
  res.write('<p><label>Time Interval&ensp;</label><select id="si" form="SAVE" name="ti" value="'+f.mqtt_timeout.toString()+'"><option '+S3+' value="60">1 min</option><option '+S1+' value="10">10 sec</option><option   '+S2+' value="0">Never</option></select></p></section>');
