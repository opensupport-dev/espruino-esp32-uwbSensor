...
if (wifi_state.BLE ==true){ESP32.enableBLE(false);}
...
var f={
    ...
    ap_port: 80,
    mode: 'APSTA',
    st_ssid: 'U+Net_WJ+SS',//'U+Net_WJ+SS', //'KT_GiGA_2G_Wave2_397D',
    st_password: 'p450505!', //'p450505!',//'6db80dd238',
    mqtt_server: "test.mosquitto.org", //"mqtt서버",
    mqtt_active: false,
...
try {
  ...
  console.log('Saving setup.ini');
  fs.writeFileSync("setup.ini", JSON.stringify(f));
}


try { 
  f = JSON.parse(fs.readFileSync("setup.ini")); 
  console.log('f:=> ', f);
  if (f.ap_ssid=='') setup = true;
}catch (e) 
{ 
...
/////////////////////////////////
if (f.ap_ssid=='') {
  f.ap_ssid = 'UWB_Sensor_' + wifi.getIP().mac.toString().substring(12,17).replace(':','').toUpperCase();
  wifi.setHostname(f.ap_ssid, function(err){
      if (err) {
          console.log("setHostname error: "+err);
      }
  });
...
function setWifiConnection(){
  setTimeout(function () {
    if ((wifi.getStatus().station !== "connected") || (wifi.getStatus().ssid ===''))
    {
      wifiAPInit();
      wifiStaConnect();
    }
...
if (setup==true) {
  ...
  {
    setWifiConnection();
  }	 // AP리스트에서 자꾸 사리지는 부분이 여기인지.... 모르겠네요. 
}
...
function wifiConnect(){
  ...
  { 
        //console.log("connected? err=", err, "info=", wifi.getIP());
        console.log("Wi-Fi connected to", f.st_ssid);
        if (f.protocol==='mqtt')
        {
...
wifi.on('sta_left',function() {
  ...
  console.log('AP was disconnected...', wifi.getIP().ip);
});
...
wifi.on('sta_joined', 	function(details) 	{
                                              ...
                                              setTimeout( function() { block_communications = 0; }, 300000); //300000 // Back to normal after 5 minutes
                                            });
wifi.on('disconnected', function(details) 	{
    troubles++;
    console.log('STA was disconnected ',details );
    if ( ((mqtt!=null) & (wifi.getStatus().mode=="STA")) 
                  || (wifi.getStatus().mode=="APSTA"))
    //if ( (mqtt!=null) & (require('Wifi').getStatus().mode=="STA") )
    {
      console.log("Wi-Fi reconnected, need to rebooting...");
      //setWifiConnection();
      ESP32.reboot();
    }
  }
...
Serial2.on('data', function(data) {
  ...
  else
  {
    ...
    else
    {
      ...
      var s=data.split(',');
      if (isNaN(parseInt(s[0],10))){T.value1=undefined;} 
      else {
...
setInterval( function() {
  if (busy === 0){
    ...
    */
    ...
    s = wifi.getStatus().station;
    ss = "";
    ap = wifi.getAPDetails().ssid;
    if ( s == 'connected' ) ss = wifi.getStatus().ssid;
    if ( wifi.getStatus().mode=='STA' ) ap = "";
   ...
   //////////////////// jayjay //////////////////
    ...
    {
        console.log("troubles: ", troubles, " errors.length: ",errors.length);
...
}, 4000); //10000 //3000 //3초 인터벌 이 부분을 웹페이지에서 변수로 받아 넣어야 합니다.
